...
def compute_two_point_pdf_and_moments(count, w_centers, w_edges, moment_orders, dtype_out=np.float32):
    """
    Convert pair counts count[r_bin, w1_bin, w2_bin] into the normalized
    two-point PDF f(t,r,w1,w2) and moments <|w1-w2|^k>(t,r).

    The PDF is normalized at fixed r:
        sum_{w1,w2} f(t,r,w1,w2) Dw1 Dw2 = 1

    Hence the moments are true conditional/statistical averages:
        <|w1-w2|^k>(t,r)
          = sum_{w1,w2} |w1-w2|^k f(t,r,w1,w2) Dw1 Dw2
          = sum_{samples at r} |w1-w2|^k / number_of_samples_at_r.
    """
    moment_orders = np.asarray(moment_orders, dtype=np.int64)
    if moment_orders.ndim != 1 or moment_orders.size == 0:
        raise ValueError("moment_orders must be a non-empty one-dimensional sequence.")
    if np.any(moment_orders < 0):
        raise ValueError("moment_orders must contain non-negative integers.")

    n_r_bins = count.shape[0]
    dw = float(w_edges[1] - w_edges[0])
    if dw <= 0.0:
        raise ValueError("w_edges must be strictly increasing.")

    count_r = count.sum(axis=(1, 2)).astype(np.float64)

    pdf = np.zeros(count.shape, dtype=dtype_out)
    for rb in range(n_r_bins):
        if count_r[rb] > 0.0:
            pdf[rb, :, :] = (count[rb, :, :] / (count_r[rb] * dw * dw)).astype(dtype_out)

    abs_dw = np.abs(w_centers[:, None] - w_centers[None, :])
    moments = np.full((moment_orders.size, n_r_bins), np.nan, dtype=dtype_out)

    for m, k in enumerate(moment_orders):
        weight = abs_dw ** int(k)
        for rb in range(n_r_bins):
            if count_r[rb] > 0.0:
                moments[m, rb] = (np.sum(count[rb, :, :] * weight) / count_r[rb]).astype(dtype_out)

    return pdf, moments, count_r.astype(np.int64), moment_orders
...
def _compute_conditional_pdf_selected_timesteps_parallel_impl(
    ...
    try:
        ...
        print(f"Moment orders for <|w1-w2|^k>(t,r): {tuple(moment_orders)}")
        print(f"Using Re = {Re}")
        ...
        for n in timesteps:
            ...
            # Two-point PDF and structure-function-like moments.
            # f_pdf is normalized for each r_bin so that
            # sum_{w1,w2} f_pdf[r,w1,w2] * Dw * Dw = 1.
            f_pdf, abs_dw_moments, r_sample_count, moment_orders_arr = (
                compute_two_point_pdf_and_moments(
                    count=count,
                    w_centers=w_centers,
                    w_edges=w_edges,
            ...
            with Dataset(output_file, "w") as out:
                ...
                out.createDimension("moment_order", len(moment_orders_arr))
                ...
                vmoment_orders = out.createVariable("moment_orders", "i8", ("moment_order",))

                vf_pdf = out.createVariable(
                    "f_pdf",
                    "f4",
                    ("r_bin", "w1_bin", "w2_bin"),
                    zlib=True,
                ...
                v_abs_dw_moments = out.createVariable(
                    "abs_dw_moments",
                    "f4",
                    ("moment_order", "r_bin"),
                    zlib=True,
                ...
                vr_sample_count = out.createVariable(
                    "r_sample_count",
                    "i8",
                    ("r_bin",),
                    zlib=True,
                ...
                vf_pdf.long_name = "normalized two-point PDF f(t,r,w1,w2)"
                vf_pdf.normalization = "sum_{w1,w2} f_pdf[r,w1,w2] * Dw * Dw = 1 for every populated r_bin"
                v_abs_dw_moments.long_name = "moments <|w1-w2|^k>(t,r) computed from f_pdf"
                v_abs_dw_moments.formula = "sum_{w1,w2} |w1-w2|^k f_pdf(t,r,w1,w2) Dw Dw"
                vr_sample_count.long_name = "total number of sample pairs in each r_bin"
                vcount.long_name = "raw sample count in each (r,w1,w2) bin"

                vr_edges[:] = r_edges
                vw_edges[:] = w_edges
                vr_centers[:] = r_centers
                vw_centers[:] = w_centers
                vmoment_orders[:] = moment_orders_arr

                vf_pdf[:, :, :] = f_pdf
                v_abs_dw_moments[:, :] = abs_dw_moments
                vr_sample_count[:] = r_sample_count
                ...
                out.dw = float(w_edges[1] - w_edges[0])
                out.n_bins = int(n_bins)
                out.moment_orders = ",".join(str(int(k)) for k in moment_orders_arr)
                out.Re = float(Re)
                ...
                out.description = (
                    "Conditional averages and two-point PDF: "
                    "f_pdf(t,r,w1,w2) is the normalized two-point PDF from raw counts; "
                    "abs_dw_moments[k,r]=<|w1-w2|^k>(t,r); "
...
def compute_conditional_pdf_selected_timesteps_parallel(
    ...
    """
    Compute conditional averages, the normalized two-point PDF, and
    moments <|w1-w2|^k>(t,r) for selected timesteps.
    ...
    New PDF/statistics output:
      - f_pdf[r,w1,w2]: normalized two-point PDF at fixed r.
      - abs_dw_moments[k_index,r]: <|w1-w2|^k>(t,r), with k stored
        in the variable moment_orders.
      - r_sample_count[r]: number of sampled pairs used at each r.
